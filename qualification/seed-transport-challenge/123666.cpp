...
Assignment assign_comb(const vector<int>&chosen,int depth,double cap,uint64_t seed,int steps,bool adaptive_floor=false){
    auto pieces=make_comb_pieces(chosen,depth);int n=(int)pieces.size();mt19937_64 rng(seed+991*chosen.size()+37*depth);
...
int main(){
    ...
    if(short_mode&&C==8&&354781<=M&&M<=399301&&br.cost>=30000){
        ...
        if((int)opts.size()>16){
            ...
            if(static_bound<br.cost){
                ...
                Result hr=evaluate_candidate(hg);evaluated++;
    ...
    if(short_mode&&C==6&&376196<=M&&M<=427656&&br.cost>=100000){
        ...
        if((int)opts.size()>1){
            ...
            Result ar=evaluate_candidate(ag);evaluated++;
    ...
    if(short_mode&&C==7&&340735<=M&&M<=387307&&br.cost>=45000){
        ...
        for(int w=0;w<hw;w++)hthreads.emplace_back([&]{for(;;){int q=hnext.fetch_add(1);if(q>=(int)highcap.size())break;hr[q]=evaluate_candidate(highcap[q]);}});
