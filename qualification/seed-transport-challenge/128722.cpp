...
int main(){
    ...
    // Low-M boards run full sims in the base pass, but those v23 sims cost
    // ~M ticks each and, on a slow/loaded machine, can consume the whole
    // budget before the adaptive stage's (cheap, sim-free) screening even
    // starts — collapsing its pool to a handful and dropping the winning
    // comb.  Cap base full-sim time so the adaptive stage always gets its
    // screening window; the analytic candidates (whole/balanced4/thirds)
    // finish instantly and carry a valid fallback regardless.  Any
    // non-analytic candidate skipped here keeps its 1/8 preview so it can
    // still be exact-checked at the deferred step.
    double base_budget=1.15;
    const char* bb_env=getenv("ORACLE_TIME_BUDGET");
    if(bb_env&&*bb_env)base_budget=stod(bb_env);
    auto base_soft=wall0+chrono::duration<double>(base_budget*0.55);
    for(int w=0;w<workers;w++)threads.emplace_back([&]{
        for(;;){
            ...
            bool analytic=analytic_label(cg);
            if((base_scaled_only||(!analytic&&chrono::steady_clock::now()>base_soft))&&!analytic){
    ...
    // Deferred exact check of the best base preview: if a base candidate was
    // skipped (high-M scaled path, or low-M base soft-cap) and its 1/8
    // preview still beats the incumbent with time to spare, spend the one
    // exact sim on it now.  On a fast judge this reproduces the old base
