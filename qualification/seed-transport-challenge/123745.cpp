...
int main(){
    ...
    if(C==5&&539216<=M&&M<=618806&&getenv("ORACLE_STAGGER_C5")&&br.cost>=30000){
        ...
        for(auto&t:sts)t.join();evaluated+=(int)pool.size();
        ...
        for(auto&t:fts)t.join();evaluated+=(int)feedback.size();
    ...
    if(short_mode&&C==7&&340735<=M&&M<=387307&&br.cost>=45000){
        ...
        for(auto&t:hthreads)t.join();evaluated+=(int)highcap.size();
    ...
    long long adaptive_trigger=12000;
    ...
    if(!disable_adaptive&&br.cost>adaptive_trigger){
        ...
        const vector<double>wide_caps={1.15,1.30,1.50,1.75,2.00};
        auto opts=strip_sets(min(3,C/2));
        ...
        vector<Cfg>kept;
        {
            ...
            vector<Cfg>lo,hi;
            for(auto&c:kept)(c.capi<=1?lo:hi).push_back(c);
            kept.clear();
        ...
        vector<Result>wr(kept.size());vector<char>done(kept.size(),0);
        {
            ...
            for(int w=0;w<ww;w++)wts.emplace_back([&]{
                for(;;){
                    ...
                    wr[q]=evaluate_candidate(built[q],curbest.load());done[q]=1;
                    if(wr[q].L==0){
