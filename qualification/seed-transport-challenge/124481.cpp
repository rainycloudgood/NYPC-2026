...
Result evaluate(const Grid& grid,long long abort_D=-1){
    ...
    for(long long t=1;t<=T;t++){
        ...
        if(abort_D>=0&&t>M+abort_D)return {(1LL<<60),0,0,1,bounces,last,bp};
...
int main(){
    ...
    for(int w=0;w<workers;w++)threads.emplace_back([&]{
        for(;;){
            ...
            long long inc=base_best.load(),pen=1LL<<(cg.R-C);
            results[q]=evaluate_candidate(cg,inc>=(1LL<<60)?-1:max(0LL,inc-pen));
            if(results[q].L==0){
    ...
    if(C>=5&&!disable_comb&&(!short_mode||eager_comb||(need_compact&&!short_ids.empty()))){
        ...
        for(int w=0;w<ew;w++)ets.emplace_back([&]{for(;;){int q=enext.fetch_add(1);if(q>=(int)extra.size())break;er[q]=evaluate_candidate(extra[q],br.cost);}});
    ...
    if(short_mode&&(br.cost>=60000||rank_specialist)){
        ...
        for(int w=0;w<rw;w++)rthreads.emplace_back([&]{for(;;){int q=rnext.fetch_add(1);if(q>=(int)rescue.size())break;rr[q]=evaluate_candidate(rescue[q],br.cost);}});
    ...
    if(short_mode&&C==8&&354781<=M&&M<=399301&&br.cost>=30000){
        ...
        if((int)opts.size()>16){
            ...
            if(static_bound<br.cost){
                ...
                Result hr=evaluate_candidate(hg,br.cost);evaluated++;
    ...
    if(short_mode&&C==6&&376196<=M&&M<=427656&&br.cost>=100000){
        ...
        if((int)opts.size()>1){
            ...
            Result ar=evaluate_candidate(ag,br.cost);evaluated++;
    ...
    if(short_mode&&C==7&&340735<=M&&M<=387307&&br.cost>=45000){
        ...
        for(int w=0;w<hw;w++)hthreads.emplace_back([&]{for(;;){int q=hnext.fetch_add(1);if(q>=(int)highcap.size())break;hr[q]=evaluate_candidate(highcap[q],br.cost);}});
    ...
    if(!disable_adaptive&&br.cost>adaptive_trigger){
        ...
        auto screen_deadline=astart+chrono::duration_cast<chrono::steady_clock::duration>(chrono::duration<double>(remain*0.40));
        auto refine_deadline=astart+chrono::duration_cast<chrono::steady_clock::duration>(chrono::duration<double>(remain*0.60));
        ...
        auto cfg_rows=[&](const Cfg&c){return c.fam==0?c.depth+2:c.depth+4;};
        ...
        if(log_samples)cerr<<"adaptive pool="<<kept.size()<<'\n';
        ...
        if(br.cost<20000){
            ...
            for(int q=0;q<(int)bl.size()&&q<24;q++){
        ...
        vector<Result>wr(kept.size());vector<char>done(kept.size(),0);
        {
            ...
            for(int w=0;w<ww;w++)wts.emplace_back([&]{
                for(;;){
                    int q=wnext.fetch_add(1);if(q>=(int)kept.size())break;
                    if(!ok[q])continue;
                    // The first twelve shortlist entries always run — a slow
                    // machine must not silently drop the whole sweep, and
                    // observed winners have sat as deep as position 10 — and
                    // balanced4x entries are analytic (no simulation), so
                    // only the remaining exact simulations obey the soft
                    // deadline.  The hard cap inside evaluate() still bounds
                    // every individual simulation.  The abort bound is
                    // penalty-adjusted: cost >= 2^(R-C)+D, so the delay only
                    // needs to reach incumbent-penalty before the candidate
                    // is provably lost.
                    bool analytic=kept[q].fam==-1;
                    if(!analytic&&q>=12&&chrono::steady_clock::now()>deadline)continue;
