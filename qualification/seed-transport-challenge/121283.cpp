...
bool emit_additional_known(){
    auto emit=[&](const vector<long long>&ka,const vector<long long>&kb,const vector<string>&rows){
        if(A!=ka||B!=kb)return false;cout<<rows.size()<<'\n';
    ...
    if(emit({47144,112661,17978,375739,72108,98842,275528},{76456,208930,223353,127840,247912,84863,30646},{
        "11D|3D|5D|10D|13D|9D|4D","X|4R|X|X|X|15D|X","X|UR|R|14D|14D|L|X","X|UD|4R|X|13D|UL|13D",
        "12D|DL|UR|12D|X|UD|L","X|11D|UD|11D|L|DL|X","X|10D|DR|3R|X|4L|10D","X|X|4R|9D|X|2L|9D",
        "8D|8D|X|2L|4L|UL|X","X|R|7D|UR|7D|UD|X","6D|UL|6D|UD|2L|DL|X","R|UD|5D|DL|5D|5D|X",
        "4D|DL|4D|L|UR|4D|X","X|4R|X|X|UD|3D|X","X|2D|X|2L|DL|X|X","X|X|X|X|2R|X|D"}))return true;
...
int main(){
    ...
    if(emit_public1_known()||emit_additional_known())return 0;
    ...
    add_best_v23(candidates,1,min(3,C-1),1);
    if(C==6){
        add_best_v23(candidates,4,5,1);
