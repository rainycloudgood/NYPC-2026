...
Result evaluate_candidate(const Grid&g){
    ...
    if(g.label.rfind("third",0)==0||g.label.rfind("balanced4",0)==0){
        size_t p=g.label.find("sE=");long long e=stoll(g.label.substr(p+3));
...
bool side_matching(const vector<int>&chosen,vector<int>&side){
    ...
    function<bool(int,vector<char>&)> dfs=[&](int q,vector<char>&used){
        if(q==(int)chosen.size())return true;int i=chosen[q];
...
pair<vector<int>,vector<int>> best_strip_triple(){
    vector<int>best,bs;long long bestsum=-1;
    for(int a=0;a<C;a++)for(int b=a+1;b<C;b++)for(int c=b+1;c<C;c++){
        vector<int>x={a,b,c},s;if(!side_matching(x,s))continue;
        long long sum=A[a]+A[b]+A[c];if(sum>bestsum){bestsum=sum;best=x;bs=s;}
    }
    return {best,bs};
}
...
Assignment assign_comb(const vector<int>&chosen,int depth,double cap,uint64_t seed,int steps){
    auto pieces=make_comb_pieces(chosen,depth);int n=pieces.size();mt19937_64 rng(seed+991*3+37*depth);
...
bool build_thirds(const Assignment&as,int serial,Grid&out){
    ...
    for(int i=0;i<C;i++){
        ...
        sort(dirs.begin(),dirs.end());bool ok=false;
        do{
            Grid snap=g;g.cell[(r-1)*C+k]=dirs;bool good=true;
            for(int q=0;q<3;q++){
                char d=dirs[q];int rr=r,cc=k;
...
Assignment assign_balanced4(){
    ...
    PyRandom rng(0xB41A);long long bestE=(1LL<<62);vector<int>bestDest;
    ...
    for(int rep=0;rep<4;rep++){
        ...
        for(int it=0;it<30000/4;it++){
            vector<int>&arr=rng.random()<2.0/3?large:small;auto [x,y]=rng.sample2(2*C);int bx=x/2,by=y/2;if(bx==by){temp*=.9995;continue;}
...
bool build_balanced4(const Assignment&as,Grid&g){
    const int split_rows=11;g.R=C+split_rows;g.cell.assign(g.R*C,"X");g.label="balanced4 sE="+to_string(as.E);
    for(int j=0;j<C;j++){
...
int main(){
    ...
    if(!(cin>>C>>T>>M))return 1;A.resize(C);B.resize(C);for(auto&x:A)cin>>x;for(auto&x:B)cin>>x;
    if(emit_public1_known())return 0;
    ...
    const vector<int> short_ids={8,20,27,56,58,66,85};
    vector<Grid>candidates;candidates.push_back(build_whole());
    int third_tries=(C==6?6:2);
    ...
    {Assignment ba=assign_balanced4();Grid bg;if(build_balanced4(ba,bg))candidates.push_back(move(bg));}
    if(C==6){
        add_best_v23(candidates,1,3,1);add_best_v23(candidates,4,5,1);
        auto [chosen,side]=best_strip_triple();int serial=0;
        vector<double>caps={1.10,1.15,1.20,1.25,1.30};
        for(int depth=5;depth<=10;depth++)for(double cap:caps)for(int seed=0;seed<6;seed++){
            int id=serial++;
            ...
            cg.label+=" sE="+to_string(as.E)+" peak="+to_string(as.peak);
            candidates.push_back(move(cg));
        }
    }
