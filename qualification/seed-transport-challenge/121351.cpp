...
vector<pair<vector<int>,vector<int>>> strip_sets(int k){
    ...
    for(int mask=0;mask<(1<<C);mask++)if(__popcnt(mask)==k){
    ...
    sort(ranked.begin(),ranked.end());vector<pair<vector<int>,vector<int>>>out;
    for(auto&[neg,x,s]:ranked)out.push_back({move(x),move(s)});return out;
}
...
int main(){
    ...
    const vector<int> short_ids={8,13,19,20,26,27,56,58,66,85};
    ...
    auto make_comb_portfolio=[&](){
        ...
        int strip_choices=1;const char* sc_env=getenv("ORACLE_STRIP_CHOICES");
        if(sc_env&&*sc_env)strip_choices=max(1,stoi(sc_env));
        strip_choices=min(strip_choices,(int)strip_options.size());
        vector<double>caps={1.10,1.15,1.20,1.25,1.30};
        for(int option=0;option<strip_choices;option++){
            auto&chosen=strip_options[option].first;auto&side=strip_options[option].second;int serial=0;
            for(int depth=5;depth<=10;depth++)for(double cap:caps)for(int seed=0;seed<6;seed++){
                ...
                cg.label+=" set="+to_string(option)+" sE="+to_string(as.E)+" peak="+to_string(as.peak);
    ...
    if(C>=5&&!disable_comb&&(!short_mode||eager_comb||br.cost>comb_trigger)){
        ...
        for(auto&t:ets)t.join();evaluated+=(int)extra.size();
    ...
    if(short_mode&&br.cost>100000){
        vector<pair<int,int>> rescue_keys;
        if(C==5)rescue_keys={{1,26},{1,66},{3,26}};
        else if(C==6)rescue_keys={{7,85}};
        ...
        for(auto [option,id]:rescue_keys)if(option<(int)opts.size()){
            ...
            Result rr=evaluate_candidate(rg);evaluated++;
            if(log_samples)cerr<<"sample "<<rg.label<<" cost="<<rr.cost<<" E="<<rr.E<<" D="<<rr.D<<" L="<<rr.L<<'\n';
            if(rr.L==0&&better(rr,br)){best=move(rg);br=rr;log_result(best,br);}
        }
    }
