...
struct S { short y, x, last_spike; signed char vy, vx, wg, wj, hold, sh, dmg, sp; };
static int spikeOnce = 3;
static int hashSpikeChoiceOnly = 1;
...
static inline int touchAt(int y, int x, S &s, bool allowDmg) {
  ...
  for (int r = r0; r <= r1; r++)
    for (int c = c0; c <= c1; c++) {
      ...
      if (t == SPK) {
        int id = r * M + c;
        bool rememberSpike = spikeOnce == 3 ? KIND == "Choice"
                           : spikeOnce >= 2 || (spikeOnce == 1 && !allowDmg);
        if (rememberSpike && s.last_spike == id) continue;
        if (rememberSpike) s.last_spike = (short)id;
        if (s.sh > 0) s.sh--;
...
static int frontierRowLimit = -1;
static int choicePair = 1;
static int choiceKnown = 10;
static int choiceActiveCol = -1, choiceOtherCol = -1;

// Choice modules reveal both one-way shafts while we ride the jump-block
// elevator.  At the top, keep the coin-bearing shaft; when neither has a coin,
// keep the one with the better shield-minus-spike balance.  Columns are inferred
// from the observed JMP column, not from a map size or seed.
static void updateChoiceDecision(int cy, int cx) {
  if (!choicePair || KIND != "Choice") return;
  int r = (cy + 3) / 6, cc = (cx + 3) / 6;
  if (r > 2) return;
  int elev = -1, bestJ = -1;
  for (int c = max(1, cc - 2); c <= min(M - 2, cc + 2); c++) {
    int cnt = 0;
    for (int rr = 1; rr + 1 < N; rr++) if (cellT[rr * M + c] == JMP) cnt++;
    if (cnt > bestJ) { bestJ = cnt; elev = c; }
  }
  if (bestJ < 4 || elev + 4 >= M - 1) return;
  int cols[2] = {elev + 2, elev + 4};
  int coins[2] = {}, net[2] = {}, known[2] = {}, needShield[2] = {};
  for (int k = 0; k < 2; k++) {
    int balance = 0;
    for (int rr = 1; rr + 1 < N; rr++) {
      unsigned char t = cellT[rr * M + cols[k]];
      if (t != UNK) known[k]++;
      if (t == COIN) coins[k]++;
      else if (t == SHLD) { net[k]++; balance++; }
      else if (t == SPK) { net[k]--; balance--; needShield[k] = max(needShield[k], -balance); }
    }
  }
  // Do not decide from a partial glimpse; the ascent should have revealed most
  // of both shafts by the time this threshold is met.
  int needKnown = max(4, min(N - 2, choiceKnown));
  if (known[0] < needKnown || known[1] < needKnown) return;
  int pick;
  bool feasible[2] = {heldShields >= needShield[0], heldShields >= needShield[1]};
  if (feasible[0] != feasible[1]) pick = feasible[0] ? 0 : 1;
  else if (coins[0] != coins[1]) pick = coins[0] > coins[1] ? 0 : 1;
  else if (net[0] != net[1]) pick = net[0] > net[1] ? 0 : 1;
  else if (needShield[0] != needShield[1]) pick = needShield[0] < needShield[1] ? 0 : 1;
  else pick = 0;
  choiceActiveCol = cols[pick];
  choiceOtherCol = cols[pick ^ 1];
}
...
static bool computeGuide(bool frontierMode) {
  ...
  for (int r = 0; r < N; r++)
    for (int c = 0; c < M; c++) {
      ...
      if (!frontierMode) {
        if (t == COIN || t == SHLD) {
          src = !(choicePair && c == choiceOtherCol);
        }
      } else if (t == UNK && (frontierRowLimit < 0 || r <= frontierRowLimit)) {
        for (int k = 0; k < 4; k++) {
...
static inline u32 stKey(const S &s) {
  ...
  bool hashSpike = spikeOnce &&
                   (!hashSpikeChoiceOnly || spikeOnce != 3 || KIND == "Choice");
  if (hashSpike) k ^= (unsigned long long)(s.last_spike + 2) * 11400714819323198485ull;
...
int main() {
  ...
  const int PAD_FINISH = envInt("PAD_FINISH", 1);
  const int PUZZLE_DROP_MARGIN = envInt("PUZZLE_DROP_MARGIN", -1);
  const int PUZZLE_SMALL_DROP_MARGIN = envInt("PUZZLE_SMALL_DROP_MARGIN", 4);
  const int PUZZLE_SMALL_DROP_MARGIN = envInt("PUZZLE_SMALL_DROP_MARGIN", -1);
  choicePair = envInt("CHOICE_PAIR", 1);
  choiceKnown = envInt("CHOICE_KNOWN", 10);
  spikeOnce = envInt("SPIKE_ONCE", 3);
  hashSpikeChoiceOnly = envInt("HASH_SPIKE_CHOICE_ONLY", 1);

  string line;
  if (!getLine(line)) return 0;
  { istringstream is(line); if (!(is >> N >> M >> KIND)) return 0; }
  // Across six independent evaluation generations, special flight on small
  // and medium Jump maps was never needed: disabling it improved three maps
  // (including 196-9: 19230 -> 0) and tied the other nine.  Large Jump maps
  // are much wider and do require flight, so keep their full budget.
  if (KIND == "Jump" && M <= 50) specBudget = 0;
  H = 6 * N; W = 6 * M;
  ...
  while (true) {
    ...
    bool needReplan = !coasting && ((planPos >= plan.size()) ||
                      (sinceReplan >= REPLAN_PERIOD) ||
                      (coins != prevCoins) || (shields != prevShields));
    if (needReplan) {
      heldShields = shields;
      updateChoiceDecision(cy, cx);
      ...
      if (!resolved) {
        int dropMargin = PUZZLE_DROP_MARGIN;
        if (dropMargin < 0 && KIND == "Puzzle" && N < 60)
          dropMargin = PUZZLE_SMALL_DROP_MARGIN;
        if (KIND == "Puzzle" && dropMargin >= 0)
          frontierRowLimit = rc + dropMargin;
        bool hasFront = computeGuide(true);
        long long firstFrontD = (hasFront && inb(rc, cc)) ? (long long)gd[rc * M + cc] : INF;
        if (frontierRowLimit >= 0 && firstFrontD >= INF) {
          frontierRowLimit = -1;
          hasFront = computeGuide(true);
        }
        frontierRowLimit = -1;
      ...
      if (!resolved) {
        ...
        flightGuide = 0;
        if (!reach) {
          if (!PAD_FINISH) { finishNow(); return 0; }
          // With no coin/frontier left, every future min Cost(t) is at most the
          // value already banked.  Cheap zero-action frames therefore can only
          // lower the final time average.  Stop replanning and coast until the
          // normal time/action guard fires.
          coasting = true;
          needSpec = false;
          resolved = true;
        }
