...
struct S { short y, x; signed char vy, vx, wg, wj, hold, sh, dmg; };
...
static int trapAware = 1;
static int trapPenOn = 0;
static long long TRAP_PEN = 1000000000LL;
static int curR = 0, curC = 0;
static vector<int> backd;

// Backward BFS with the platformer's real move set, over whatever bq was seeded
// with. A plain 4-neighbour flood claims a shield four cells straight up is four
// steps away; it is in fact unreachable from below, and chasing it wedges us
// against the shaft forever. A jump rises at most 15px (2.5 cells), so upward
// moves are only legal from a supported cell and only two at a time. Falling and
// air-steering are free. f[X] ends up as the forward step count from X to a seed.
static void expandField(vector<int> &f) {
  for (size_t h = 0; h < bq.size(); h++) {
    int id = bq[h], r = id / M, c = id % M, d = f[id];
    int cand[5][2] = {{r, c - 1}, {r, c + 1}, {r - 1, c}, {r + 1, c}, {r + 2, c}};
    for (int k = 0; k < 5; k++) {
      int nr = cand[k][0], nc = cand[k][1];
      if (!inb(nr, nc)) continue;
      int nid = nr * M + nc;
      if (f[nid] != INF) continue;
...
static bool computeGuide(bool frontierMode) {
  ...
  if (!frontierMode && shieldFirst) {
    ...
    if (shieldCells > 0 && spikes > heldShields) {
      bq.clear();
      gd.assign(N * M, INF);
      for (int i = 0; i < N * M; i++)
  ...
  if (trapAware) {
    // Some coins sit at the bottom of a pit with no way back out. Grabbing one
    // early strands us for the rest of the run -- pits_small spent 1077 of 1140
    // frames in a shaft whose floor is 24px below a 15px jump. So prefer targets
    // we can still walk away from, and only enter a one-way pit once nothing
    // else is left (by then the sacrifice costs nothing).
    backd.assign(N * M, INF);
    vector<int> seeds(bq);
    bq.clear();
    int home = curR * M + curC;
    if (inb(curR, curC)) { backd[home] = 0; bq.push_back(home); }
    expandField(backd);
    vector<int> safe;
    for (int id : seeds)
      if (backd[id] < INF) safe.push_back(id);
    // Only shun one-way pits while something else is still on offer; once the
    // pit holds the last coin, dropping in costs us nothing we still need.
    trapPenOn = safe.empty() ? 0 : 1;
    bq = safe.empty() ? seeds : safe;
    gd.assign(N * M, INF);
    for (int id : bq) gd[id] = 0;
  }
  expandField(gd);
  return true;
}
...
static inline long long scoreState(const S &s) {
  ...
  long long trap = (trapPenOn && inb(ry, rx) && backd[ry * M + rx] >= INF) ? TRAP_PEN : 0LL;
  return g * 100000LL + v * VIS_SC + (long long)s.dmg * DMG_SC + trap;
}
...
static inline long long prio(const S &s, int depth) {
  ...
  long long trap = (trapPenOn && inb(ry, rx) && backd[ry * M + rx] >= INF) ? TRAP_PEN : 0LL;
  return g * GUIDE_W + (long long)s.dmg * DMG_COST + v * VIS_W + depth + trap;
}
...
static bool planBFS(const S &root, bool allowDmg, int cap, int maxDepth,
                    bool exploreMode, vector<int> &plan) {
  nodes.clear();
  curGen++;
...
int main() {
  ...
  DMG_COST = (long long)envInt("DMG_COST", 200000);
  ...
  shieldFirst = envInt("SHIELD_FIRST", 1);
  trapAware = envInt("TRAP_AWARE", 1);
  TRAP_PEN = (long long)envInt("TRAP_PEN", 1000000) * 1000LL;
  ...
  cur.vy = cur.vx = cur.wg = cur.wj = cur.hold = cur.dmg = 0;
  ...
  while (true) {
    ...
    if (needReplan) {
      heldShields = shields;
      curR = (cur.y + 3) / 6; curC = (cur.x + 3) / 6;
      bool haveTargets = computeGuide(false);
      bool exploreMode = false;
      if (haveTargets) {
        int rc = (cur.y + 3) / 6, cc = (cur.x + 3) / 6;
        if (!inb(rc, cc) || gd[rc * M + cc] >= INF) haveTargets = false;
      }
      if (!haveTargets) {
        if (!computeGuide(true)) { finishNow(); return 0; }
        exploreMode = true;
      }
      ...
      bool ok = planBFS(cur, false, cap, MAX_DEPTH, exploreMode, plan);
      if (!ok || plan.empty()) ok = planBFS(cur, true, cap, MAX_DEPTH, exploreMode, plan);
      if (!ok || plan.empty()) {
        noTargetStreak++;
