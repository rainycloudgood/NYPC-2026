...
!!코드 재사용과 활용을 금지합니다. 코드를 자유롭게 쓰도록 해드리고싶지만, 대회 중에 급하게 작성한 코드라 각종 문제가 발생할 수 있기에 그냥 코드를 보시면서 이런식으로 작성했고 풀이했구나! 정도로만 봐주시면 감사하겠습니다.!!

## 포함하지 않은 것

- **문제 원문, 채점 입력, 대회 제공 데이터** — 넥슨(NYPC) 자료이므로 재배포하지 않습니다. 보고싶으신 분들은 [NYPC ARCHIVE](https://nypc.github.io/2026)로 가시면 됩니다!
...
저는 앞으로 또 다른 도전을 계속하려합니다. 저에게 또 하나의 원동력을 만들어주는 기회를 만들어주신 NYPC 운영진분들 감사했습니다.
그럼, 다음 도전을 기록하는 레포에서 봐요!
