...
Assignment assign_mw(const vector<pair<int,int>>&pairs,uint64_t seed,int steps=30000){
    ...
    vector<int>perm(m);iota(perm.begin(),perm.end(),0);
    {
        ...
        for(int z=0;z<(int)(seed%7);z++)swap(perm[rng()%m],perm[rng()%m]);
    }
    vector<array<int,2>>Ls(m),Ss(m);vector<long long>got(m,0);
    for(int b=0;b<m;b++){
        int s=ss[perm[b]];
        Ls[b]={base[s],base[s]+1};Ss[b]={base[s]+2,base[s]+3};
        for(int z=0;z<2;z++){got[b]+=pieces[Ls[b][z]].amount;got[b]+=pieces[Ss[b][z]].amount;}
    }
    ...
    for(int it=0;it<steps;it++){
        ...
        bool big=rng()%3<2;
        auto&Pa=big?Ls[a]:Ss[a];auto&Pb=big?Ls[b]:Ss[b];
        int ia=(int)(rng()%2),ib=(int)(rng()%2);
    ...
    for(int b=0;b<m;b++){
        E+=Eb(b);
        for(int z=0;z<2;z++){dest[Ls[b][z]]=sb[b];dest[Ss[b][z]]=sb[b];}
    }
        ...
        if(br.cost<60000){
            ...
            for(int k=1;k<=min(2,C-2);k++)for(int v=0;v<2;v++){
                ...
                for(int sd=0;sd<2;sd++){
                    auto cand=assign_mw(pr,0xC0FFULL+v*1000003ULL+k*131+sd*997);
