...
# Rating v4 stable: public1=15,406; max-half removed; guarded adaptive tails.
import random
import itertools
import sys
...
def solve_variable23(C,T,M,A,B,max_k=3,min_k=1):
 best=None;base_seed=sum((q+1)*x for q,x in enumerate(A+B))
 for k in range(max(1,min_k),min(max_k,C-1)+1):
...
# Rate-safe tail rescue: every source and every destination has the same
# 2*(1/3)+2*(1/6) stream profile.  Therefore no burrow is overdriven and the
# destination bus never pushes back into a splitter.
def balanced4_assign(A,B,seed=0,restarts=4,steps=30000):
 C=len(A);pieces=[]
 for a in A:
  q,r=divmod(a,3);t=[q+(i<r) for i in range(3)];x=t[2]
  pieces.append([t[0],t[1],(x+1)//2,x//2])
 rng=random.Random(0xB41A+seed*1000003);best=None
 large0=[(i,q) for i in range(C) for q in (0,1)]
 small0=[(i,q) for i in range(C) for q in (2,3)]
 for _ in range(restarts):
  large=large0[:];small=small0[:];rng.shuffle(large);rng.shuffle(small)
  got=[0]*C
  for j in range(C):
   for i,q in large[2*j:2*j+2]+small[2*j:2*j+2]:got[j]+=pieces[i][q]
  cur=sum(abs(got[j]-B[j]) for j in range(C));temp=max(100.0,cur/max(1,C))
  for __ in range(steps//restarts):
   arr=large if rng.random()<2/3 else small;x,y=rng.sample(range(2*C),2)
   bx,by=x//2,y//2
   if bx==by:continue
   sx,qx=arr[x];sy,qy=arr[y];vx,vy=pieces[sx][qx],pieces[sy][qy]
   old=abs(got[bx]-B[bx])+abs(got[by]-B[by])
   nx,ny=got[bx]-vx+vy,got[by]-vy+vx;new=abs(nx-B[bx])+abs(ny-B[by]);d=new-old
   if d<=0 or rng.random()<pow(2.718281828,-d/max(1.0,temp)):
    arr[x],arr[y]=arr[y],arr[x];got[bx],got[by]=nx,ny;cur+=d
   temp*=.9995
  if best is None or cur<best[0]:
   target=[[-1]*4 for _ in range(C)]
   for j in range(C):
    for i,q in large[2*j:2*j+2]+small[2*j:2*j+2]:target[i][q]=j
   best=cur,target
 return best

def solve_balanced4_bus(C,T,M,A,B):
 sols=[]
 for seed in range(1):
  e,target=balanced4_assign(A,B,seed);sols.append((e,target))
 e,target=min(sols,key=lambda z:z[0]);split_rows=11;R=C+split_rows
 g=[['X']*C for _ in range(R)]
 for j in range(C):
  br=split_rows+1+j
  for c in range(C):
   g[br-1][c]=('R' if c<j else ('L' if c>j else tok(R+1-br,'D')))
 def send(r,c,j):
  if not(1<=r<=split_rows and 0<=c<C) or g[r-1][c]!='X':return False
  g[r-1][c]=tok(split_rows+1+j-r,'D');return True
 for i in range(C):
  r=3+3*(i%3);side='R' if i<C-1 else 'L';dc=1 if side=='R' else -1
  ...
  for (rr,cc),j in zip(starts,target[i]):
...
def main():
    ...
    third_tries=(6 if C==6 and M<=376195 else
                 (6 if C==9 and risk_band in (2,3) else
                  (6 if C==8 and risk_band==3 else
                  (6 if C==6 and risk_band==4 else
    ...
    # Largest-source half + remaining thirds.  Exact intermediate logs showed
    # catastrophic L on the former C=7/band1 and C=8/band2 branches, so those
    # are deliberately excluded even when their modeled E looks excellent.
    # Exact official-distribution tests found catastrophic L across C=5,6,7,8
    # (20-40% failure rates in sampled enabled bands).  Static E cannot detect
    # this failure mode, so max-half is disabled globally for stability.
    use_maxhalf = False
    if use_maxhalf:
    ...
    if risk_level==2:
        ...
        if vz is not None:
            ...
            accept=(vest < chosen_est)
            # C=5 high-M layouts show noticeable backpressure model error.
            # Demand a material modeled win instead of chasing a 1-2% gain.
            if C==5:accept=(vest*10 <= chosen_est*9)
            if accept:R,g,chosen_est=vr,vg,vest
    # C=9 / 60-80%ile is the remaining rating tail.  Broader 2/3 mixtures
    # help only when their modeled advantage is overwhelming; exact offline
    # simulation found the 70% gate conservative (no regressions in the
    # sampled official distribution).  Search k=4..8 only, preserving the
    # normal k<=3 decision above and avoiding duplicate runtime.
    if C==9 and risk_band==3 and chosen_est>=50000:
        vz=solve_variable23(C,T,M,A,B,max_k=C-1,min_k=4)
        if vz is not None:
            vr,vg,ve=vz;vest=(1 << (C+4))+max(ve,4)
            if vest*10 <= chosen_est*7:R,g,chosen_est=vr,vg,vest
    # C=6 / top-M tail remains geometrically stable for the two wider
    # mixtures (k=4,5), unlike C=8.  Exact official-distribution samples had
    # no loss/regression, so retain the ordinary modeled-cost gate.
    if C==6 and risk_band==4:
        vz=solve_variable23(C,T,M,A,B,max_k=C-1,min_k=4)
        if vz is not None:
            vr,vg,ve=vz;vest=(1 << (C+4))+max(ve,4)
            if vest < chosen_est:R,g,chosen_est=vr,vg,vest
    # C=6 / low-M rating rescue.  Wider k=4..5 mixtures are exact-tested
    # safe here, but only enter when their modeled objective beats the current
    # plan; this rejects the easy low-error cases where they regress sharply.
    if C==6 and risk_band==0 and chosen_est>=50000:
        vz=solve_variable23(C,T,M,A,B,max_k=C-1,min_k=4)
        if vz is not None:
            vr,vg,ve=vz;vest=(1 << (C+4))+max(ve,4)
            if vest < chosen_est:R,g,chosen_est=vr,vg,vest
    # Expensive only on the current >100K tail.  Unlike the old arbitrary
    # weighted-four candidate, this profile has exact rate 1 at every burrow;
    # sampled exact simulation therefore matches E+2^(R-C) and always has L=0.
    if safe_est>100000:
        bz=solve_balanced4_bus(C,T,M,A,B)
        if bz is not None:
            br,bg,be=bz;best=(1<<(br-C))+max(be,4)
            if best<chosen_est:R,g,chosen_est=br,bg,best
    print(R)
